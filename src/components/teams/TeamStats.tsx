        ...
        <CardContent className={compactMode ? 'p-2 pt-2' : 'pt-6'}>
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <h3 className={`${compactMode ? 'text-lg' : 'text-2xl'} font-bold`}>
                {isLoading ? <LoadingSpinner size={compactMode ? 3 : 4} /> : displayTeams.filter(t => {
                  const winPercent = t.WinPercentage !== undefined 
                    ? (typeof t.WinPercentage === 'string' ? parseFloat(t.WinPercentage) : t.WinPercentage)
                    : t.winPercentage !== undefined 
                      ? parseFloat(t.winPercentage) 
                      : 0;
